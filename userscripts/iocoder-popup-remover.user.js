...
// @version      2.2.0
// @description  解锁 doc.iocoder.cn 付费文档：注入 VIP cookie + 拦截 auth 校验(XHR原生层)，SPA 路由切换下稳定解锁
// @author       YourName
...
(function() {
    ...
    // ========== 第一层：注入 VIP cookie ==========
    // document-start 执行，先于网站 app.js 的 afterEach 读取 cookie，让 d()=true
    ...
    function hijackXHR() {
        ...
        // 用 WeakSet 标记需要伪造的 xhr 实例，避免泄漏引用
    ...
    // 第二层最先：XHR 是浏览器原生，document-start 立即可拦截（不依赖任何库加载）
    hijackXHR();
    ...
    // SPA 路由切换不触发 document-start，补注入 cookie
    const reInject = () => {
